...
cleanDocker:
	-docker stop $(docker-compose ps -q backend) $(docker-compose ps -q frontend) || true
	-docker rm $(docker-compose ps -q backend) $(docker-compose ps -q frontend) || true
	-docker rmi $(docker images -q backend) $(docker images -q frontend) || true
...
.PHONY: check-npm check-nextjs kill-ports run-backend run-frontend run
